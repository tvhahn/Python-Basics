...
# FIT the model on 33%
clf = support_vector(X_train, y_train, X_test)
# test_model_lg(X,y) # test general model

# SAVE model in a pickle
save_classifier = open("svm_clf.pickle","wb")
...
p_submit = clf.predict(test)
prediction = pd.DataFrame()
prediction['PassengerId'] = test_pid['PassengerId']
prediction['Survived'] = p_submit

# output the predictions to a csv, ready to import to Kaggle
prediction.to_csv('svm_pred.csv',index=False)